...
class TRPO(ActorCriticRLModel):
    ...
    """
    def __init__(self, policy, env, gamma=0.99, timesteps_per_batch=512, max_kl=0.01, cg_iters=10, lam=0.98,
                ...
                while True:
                    ...
                    for k in range(self.g_step):
                        ...
                        for i, ob in enumerate(observation):
                            processed = process_image(ob)
                            processed = np.stack((processed, processed, processed, processed), axis=2)
                            processed_obs.append(processed)
                        observation = np.array(processed_obs)

                        # d1, d2, d3, d4 = observation.shape
                        # observation = observation.reshape((d1, d2 * d3 * d4))
                    ...
                    if self.using_gail:
                        ...
                        for ob_batch, ac_batch in dataset.iterbatches((observation, action),
                            ...
                            processed_obs = []
                            for i, ob in enumerate(ob_expert):
                                processed = process_image(ob)
                                processed = np.stack((processed, processed, processed, processed), axis=2)
                                processed_obs.append(processed)
                            ob_expert = np.array(processed_obs)
                            # d1, d2, d3, d4 = ob_expert.shape
                            # ob_expert = ob_expert.reshape((d1, d2 * d3 * d4))
